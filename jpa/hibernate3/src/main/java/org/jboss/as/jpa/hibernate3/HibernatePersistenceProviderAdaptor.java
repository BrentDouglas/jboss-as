...
public class HibernatePersistenceProviderAdaptor implements PersistenceProviderAdaptor {
    ...
    private static final String DEFAULT_CACHE_MANAGER = "java:jboss/infinispan/hibernate";
    ...
    public Iterable<ServiceName> getProviderDependencies(final PersistenceUnitMetadata pu) {
        ...
        if (Boolean.parseBoolean(properties.getProperty("hibernate.cache.use_second_level_cache"))) {
            ...
            getPropertyOrDefault(properties, "hibernate.cache.region_prefix", pu.getScopedPersistenceUnitName());
            final String regionFactory = getPropertyOrDefault(properties, "hibernate.cache.region.factory_class", DEFAULT_REGION_FACTORY);
            final String cacheManager = getPropertyOrDefault(properties, "hibernate.cache.infinispan.cachemanager", DEFAULT_CACHE_MANAGER);

            if (regionFactory.equals(DEFAULT_REGION_FACTORY)) {
                // Set infinispan defaults
                final String container = getPropertyOrDefault(properties, "hibernate.cache.infinispan.container", DEFAULT_CACHE_CONTAINER);
    ...
    private String getPropertyOrDefault(final Properties properties, final String property, final String defaultValue) {
